...
import com.lagou.edu.mvcframework.Interceptor;
import com.lagou.edu.mvcframework.SecurityInterceptor;
...
public class LgDispatcherServlet extends HttpServlet {
    ...
    //拦截器链
    private List<Interceptor> interceptors = new ArrayList<>(2);//存储拦截器
    ...
    private void initInterceptors() {
        interceptors.add(new SecurityInterceptor(ioc));//这里将Security拦截器加入
    }
...
//        method.invoke() //
        ...
        //拦截器验证
        doInterceptor(req, resp,handler);
    ...
    private void doInterceptor(HttpServletRequest req,HttpServletResponse resp,Handler handler) throws IOException {
        if(interceptors.isEmpty()){return;}

        for (Interceptor interceptor : interceptors) {
            String url = req.getRequestURI();
            if (interceptor.support(url)){
                try {
                    boolean b = interceptor.preInterceptor(req, resp, url, handler);
                    if(!b){return;}
                    interceptor.doInterceptor(req,resp,url,handler);
                } catch (IOException e) {
                    System.out.println("拦截异常,errMsg:"+e.getMessage());
                    throw e;
                }
            }
        }
